...
export const setMediafileOnAsset = async (
  _dataSources: DataSources,
  _assets: Array<Entity>,
  _frameId: string
) => {
  const updatedAssets: Array<Entity> = _assets;
  let components = await _dataSources.EntitiesAPI.getRelationOfType(
    _frameId,
    RelationType.Components
  );
  components = relationsWithExcludedCollections(components, [
  ...
  for (const [index, _component] of components.entries()) {
    if (
      _component.setMediafile &&
      _component.setMediafile != 1 &&
  ...
  if (itemsToUpdate.length >= 1) {
    for (const item of itemsToUpdate) {
      if (
        updatedAssets[item.index] &&
        updatedAssets[item.index].id === item.id
      ) {
        updatedAssets[item.index].primary_mediafile_location =
          item.mediaFile.original_file_location;
        ...
        updatedAssets[item.index].primary_transcode =
          item.mediaFile.transcode_filename;
        updatedAssets[item.index].primary_transcode_location =
          item.mediaFile.transcode_file_location;
      }
    }
  }
...
export const createRelationOfId = (
  _entityId: string,
...
export const addTimingsToRelation = (
  _relation: Relation,
  _timings: Record<'start' | 'zoom' | 'end', number>
) => {

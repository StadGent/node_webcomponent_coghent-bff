...
import {
  addTimingsToRelation,
  createRelationOfId,
} from '../resolvers/relationMetadata';
import {
  Entity,
  KeyValuePair,
  Metadata,
...
export const createRelationTiming = (
  _startTime: number,
  _duration: number
): Relation => {
...
export const createRelationsOfStorybox = (_storyboxBuild: StoryboxBuild) => {
  ...
  for (const _asset of _storyboxBuild.assetTimings!) {
    ...
    relationWithTimings.value = findDescriptionOfAsset(
      _asset?.key!,
      _storyboxBuild.assetDescriptions as KeyValuePair[]
    );
...
export const findDescriptionOfAsset = (
  assetKey: string,
  ...
  if (assetDescriptions) {
    description =
      assetDescriptions.find((description: any) => description.key === assetKey)
        ?.value || '';
  }
  return description;
};
...
export const createRelationTypeFromData = (
  _type: RelationType,
  _key: string,
  _keyPrefix: `entities/`
) => {
